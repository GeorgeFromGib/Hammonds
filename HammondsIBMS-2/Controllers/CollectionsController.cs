...
namespace HammondsIBMS_2.Controllers
{
    public class CollectionsController : IbmsBaseController
    {
        ...
        {
            return rolledTrans.Where(c=>c.Amount!=0).OrderBy(c=>c.CustomerAccountId).ThenBy(c=>c.GroupId).ThenBy(c=>c.AccountTransactionType).Select(c => new CollectionVM
            {
        ...
        private void InitialiseCustomerAccountsViewBagAndAddSelector(int customerId)
        {
            ...
            ViewBag.CustomerAccounts = AddAllSelector<dynamic>(new {CustomerId = -1, Description = "-----"},ViewBag.CustomerAccounts);
        }

        private void InitialiseCustomerAccountsViewBag(int customerId)
        {
            ViewBag.CustomerAccounts = _customerService.GetCustomerAccounts(customerId).Select(c => new { c.CustomerAccountId, Description = c.AccountType.ToString() });
        }
    }
